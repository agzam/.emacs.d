...
;; Soft: the live session always has elpaca; the buttercup sandbox, which
;; loads this file to spec the pure parts, does not.
...
Never signals: the finished/failed subscribers run inside elpaca's process
sentinels, where an unwritable progress file must not be able to wedge the
whole queue - and the tick's own error handler emits, so a signaling emit
would turn one tick error into a forever-looping poll.  A tail gone blind
is covered on the caller's side by its pid liveness check and deadline."
...
forever, spamming the session and leaving the tail staring at dead air until
its deadline.  A tick error instead lands the terminal UPDATE-ERROR marker
and dismantles the run."
  (condition-case err
...
returns immediately; the caller tails the progress file for a terminal
UPDATE-DONE / UPDATE-FAILED line.

Returns the progress-file path this run tees to: LOGFILE for a fresh run,
or the in-flight run's own file when an update is already going - the
caller re-attaches by tailing the returned path from the top, replaying
progress so far, rather than erroring out or racing a second update.
Returns nil when the update could not start; LOGFILE then holds the
UPDATE-ERROR line."
  (if (and elpaca-live-update--poll
           (memq elpaca-live-update--poll timer-list))
      elpaca-live-update--log
    ;; A non-nil poll whose timer is no longer armed is the wreck of an
    ;; interrupted run (a mid-run reload, a cancelled timer): dismantle it
    ;; rather than refuse every update until a restart.
    (when elpaca-live-update--poll (elpaca-live-update--cleanup))
    (condition-case err
        (progn
