;;; modules/lookup/autoload/online.el -*- lexical-binding: t; -*-

;; Vendored from Doom :tools lookup autoload/online.el, plus-free.  The
;; counsel/helm "special provider frontends" (+lookup--online-backend-google
;; and -duckduckgo) stayed behind - ivy/helm never existed here, so the
;; provider alist carries plain URL strings only.
...
time, the provider from `lookup-provider-url-alist'.  In subsequent calls,
reuse the previous provider.  With a non-nil prefix argument, always prompt
for the provider.
