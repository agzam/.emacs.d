;;; modules/clojure/config.el -*- lexical-binding: t; -*-

;; doom.d custom/clojure.  Renames (plus-ban/shadowing, collision-checked
;; against the packages): cider-storm-storm-start-gui+ ->
;; cider-storm-start-gui-styled and lsp-clojure-show-project-tree+ ->
;; lsp-clojure-project-tree-toggle (both wrap real package commands),
;; clojure-project-root-path+ -> clojure-project-root-path-poly,
;; +cider-complete-at-point -> cider-complete-at-point-maybe,
;; +clojure/* -> bare names, spacehammer-monroe--* -> hammerspoon-monroe-*
;; (writing-port precedent), separedit--*-clj-str-delimeters ->
;; clj-str-*-delimiters (package-namespace squat + typo).
;; Dropped as doom.d rot (git-resurrectable): clojure-enable-fancify-symbols
;; (spacemacs-era free variable, no such var in clojure-mode), the duplicate
;; cider-repl-display-in-current-window setq; cljr-favor-prefix-notation
;; moved home to the clj-refactor block.  Projectile root markers ->
;; project.el's project-vc-extra-root-markers (projectile dropped
;; config-wide).
...
(after! project
  (dolist (f '("project.clj" "build.boot" "deps.edn"))
    (add-to-list 'project-vc-extra-root-markers f)))
...
(use-package clojure-mode
  ...
  (add-hook! (clojurescript-mode clojure-ts-clojurescript-mode)
    (defun activate-clojurescript-dash-docset-h ()
  ...
  (after! lsp-clojure
    ...
    (add-hook! 'lsp-mode-hook
      (defun clj-set-completion-at-point-h ()
  ...
  (add-hook! (clojurescript-mode clojure-ts-clojurescript-mode)
    #'visual-wrap-prefix-mode
    (defun add-reframe-regs-to-imenu-h ()
      (add-to-list
...
(use-package cider
  ...
  ;; In cider-debug-mode, override evil keys to not interfere with debug keys
  (after! evil
    (add-hook! 'cider--debug-mode-hook
      (defun cider-debug-setup-h ()
  ...
  (map! (:localleader
         ;; shared between clojure modes and edn
         (:map (clojure-mode-map
               ...
               (:prefix ("s" . "repl")
                        ...
                        "q" #'cider-quit
                        "R" #'cider-restart
                        "B" #'cider-switch-to-repl-and-switch-ns
         ...
         (:map (clojure-mode-map
               ...
               (:prefix ("n" . "namespace")
                        ...
                        "r" #'cider-ns-refresh)
               (:prefix ("p" . "print")
                        "t" #'lsp-clojure-project-tree-toggle)
               (:prefix ("s" . "repl")
                        "n" #'cider-repl-set-ns
                        "r" #'cider-ns-refresh)
               (:prefix ("t" . "test")
...
(after! separedit
  ...
  (add-to-list
   ...
   '(:header "(str\s+\""
     ...
     :delimiter-remove-fn clj-str-remove-delimiters
     :delimiter-restore-fn clj-str-restore-delimiters
     :edit-mode markdown-mode))

  (defadvice! fix-separedit-region-for-clj-a (block-info-fn)
...
(use-package cider-storm
  :ensure (cider-storm :host github :repo "jpmonettas/cider-storm")
  :after clojure-mode
  :config
...
(use-package monroe
  ...
  (when (eq system-type 'darwin)
    ...
    ;; Inert until the lua module ports fennel-mode - map! waits on the
    ;; feature, the hook var costs nothing.
...
(use-package logview
  :ensure (logview :host github :repo "doublep/logview")
  :defer t)

(use-package datetime
  :ensure (datetime :host github :repo "doublep/datetime")
  :defer t)
