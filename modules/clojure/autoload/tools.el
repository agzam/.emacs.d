;;; modules/clojure/autoload/tools.el -*- lexical-binding: t; -*-

;;;###autoload
(defun cider-storm-start-gui-styled ()
  "Start the FlowStorm GUI with the configured theme and styles.
Distinct from the package's own `cider-storm-storm-start-gui', which it
wraps with the local-connect theme/styles arguments."
  (interactive)
...
(defun clj-dev-tool-profile ()
  "Profile the sexp at point with clj-async-profiler, open the flamegraph."
  (interactive)
...
(defun clj-dev-tool-bench ()
  "Quick-bench the sexp at point with criterium."
  (interactive)
  (let* ((current-ns (cider-current-ns))
         (form (save-excursion
                 (forward-char 1)
...
(defun cider-storm-switch-to-gui ()
  "Focus the FlowStorm debugger window via Hammerspoon."
  (interactive)
  ;; calling shell-command synchronously is very slow; this works much
  ;; faster, but suppress the output since we don't care about it
...
;;;###autoload
(defun lsp-clojure-project-tree-toggle ()
  "Pop to the clojure-lsp project tree, or show it if not visible.
Distinct from the package's `lsp-clojure-show-project-tree', which it wraps."
  (interactive)
