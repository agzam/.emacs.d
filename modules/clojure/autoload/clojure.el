;;; modules/clojure/autoload/clojure.el -*- lexical-binding: t; -*-

;;;###autoload
(defun clojure-lookup-handlers-h ()
  "Wire the lsp/cider lookup handlers for all clojure modes."
...
;;;###autoload
(defun clj-str-remove-delimiters (_ &optional _)
  "Strip the leading/trailing quotes of a Clojure (str ...) block's lines."
  (save-excursion
    (replace-regexp-in-region "^\s*\"" "" (point-min))
    (replace-regexp-in-region "\"\s*$" "" (point-min))))

;;;###autoload
(defun clj-str-restore-delimiters (&optional _)
  "Re-quote the lines of an edited Clojure (str ...) block."
  (save-excursion
