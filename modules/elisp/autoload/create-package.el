;;; modules/elisp/autoload/create-package.el --- new-package scaffolding -*- lexical-binding: t; -*-
;;; Commentary:
;; Scaffolds a package repo from templates/ (self-rolled - no dependency on
;; Doom's file-templates).  Deviation from doom.d: the clone-root read of
;; magit-clone-default-directory is bound-and-true-p guarded - it was a
;; void-variable until magit happened to load.
;;; Code:
...
Creates a standard project structure with GitHub Actions CI and a
buttercup test skeleton under the git clone root."
  ...
  (let* ((name (string-trim (replace-regexp-in-string "\\.el$" "" name)))
         ...
         (root (expand-file-name
                gh-user (or (bound-and-true-p magit-clone-default-directory)
                            "~/GitHub/")))
