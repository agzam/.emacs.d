;;; modules/web-browsing/autoload/eww.el -*- lexical-binding: t; -*-
...
To be used by `eww-after-render-hook'."
  ;; doom.d compared the title with (eq "") - never true for a fresh
  ;; string, so untitled pages got "* # eww*" names; fixed here
  (let* ((title (plist-get eww-data :title))
         (name (if (or (null title) (string-empty-p title))
...
(defun eww-make-readable-a (fn charset url document point buffer)
  ...
  (let* ((tmp-in (make-temp-file "src" nil ".html"))
         ...
         (asset-dir (expand-file-name "modules/web-browsing/readable/"
                                      user-emacs-directory))
         ;; npm must never write node_modules into the config tree; the
         ;; script and its deps live in a cache workdir instead.
         (work-dir (expand-file-name "readable/" doom-cache-dir))
         (script (expand-file-name "extract.cljs" work-dir)))
    (unless (file-directory-p (expand-file-name "node_modules" work-dir))
      (make-directory work-dir t)
      (dolist (f '("extract.cljs" "package.json"))
        (copy-file (expand-file-name f asset-dir)
                   (expand-file-name f work-dir)
                   t))
      ;; doom.d handed shell-command format args without `format' - the
      ;; bootstrap never ran
      (let ((default-directory work-dir))
        (shell-command
         (format "%1$s install -g nbb && %1$s install"
                 (executable-find "npm")))))
    (unwind-protect
        (progn
...
;; blindly copy it!
;;;###autoload
(defun shr-put-sliced-image (spec alt &optional flags)
