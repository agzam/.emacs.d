...
(defun reload-dir-locals (proj)
  "Re-read PROJ's .dir-locals and re-apply in all of its file buffers.
...
(defun parse-git-url (url)
  "Parse git URL into a (:host :org :repo) plist."
...
(defun bisect-github-url (url)
  "Return plist with parts of GitHub URL."
...
If URL is a link to a file, extract its raw form and open it in a buffer."
  (interactive)
  (let* ((url (or url
                  ;; basic
...
(defun forge-visit-topic-via-url (&optional url)
  "Open a Forge Topic buffer or the raw file, based on GitHub URL."
  (interactive)
  (require 'forge)
  (require 'deferred)
  ;; ghub moved ghub-fetch-issue/-pullreq here; nothing else requires it
...
(defun make-path (&rest parts)
  "Join PARTS into an expanded file path."
...
(defun update-ssh-auth-sock-h ()
  "Update SSH_AUTH_SOCK to the current SSH agent socket.
Useful for long-running Emacs sessions in tmux where the forwarding
socket changes between SSH connections. Only runs in terminal mode over
SSH."
