;;; modules/python/autoload/python.el -*- lexical-binding: t; -*-

;;;###autoload
(defun python-lookup-handlers-h ()
  "Wire the lsp lookup handlers for python modes."
...
(defun python--fqn-via-lsp ()
  ...
  (condition-case nil
      (let* ((sym (thing-at-point 'symbol t))
             ...
             ;; dash-free rewrite of doom.d's `-some->' hover chain.
             (hover (when-let* ((req (lsp--make-request
                                      "textDocument/hover"
                                      (lsp--text-document-position-params)))
                                (resp (lsp--send-request req))
                                (cnt (lsp:hover-contents resp)))
...
;;;###autoload
(defun python-fully-qualified-symbol-at-point (&optional callback)
